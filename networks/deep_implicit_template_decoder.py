...
class Generator(nn.Module):
    ...
    def forward(self, input, step=1.0):
        ...
        inverse_warped_xyzs = []
        for s in range(self.steps):
            state = self.lstm(torch.cat([code, warped_xyz], dim=1), states[-1])
            if state[0].requires_grad:
                state[0].register_hook(lambda x: x.clamp(min=-10, max=10))
            a = self.out_layer_coord_affine(state[0])
            tmp_warped_xyz = torch.addcmul(a[:, 3:], (1 + a[:, :3]), warped_xyz)

            warping_param.append(a)
            states.append(state)
            if (s+1) % (self.steps // 4) == 0:
                inverse_warped_xyzs.append(tmp_warped_xyz)
            warped_xyz = tmp_warped_xyz

        if step < 1.0:
            xyz_ = input_bk[:, -3:]
            warped_xyz = warped_xyz * step + xyz_ * (1 - step)

        return warped_xyz, warping_param, inverse_warped_xyzs
...
class InverseImplicitFun(nn.Module):
    def __init__(self, latent_size, hidden_size=512):
        ...
        self.fc1 = nn.Linear(latent_size+3, hidden_size)
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        self.fc3 = nn.Linear(hidden_size, hidden_size)
        self.fc4 = nn.Linear(hidden_size, hidden_size)
        self.fc5 = nn.Linear(hidden_size+latent_size+3, hidden_size)
        self.fc6 = nn.Linear(hidden_size, hidden_size)
        self.fc7 = nn.Linear(hidden_size, hidden_size)
        self.fc8 = nn.Linear(hidden_size, 3)

    def forward(self, interm_input):

        x1 = self.fc1(interm_input)
        ...
        x4 = torch.cat((x4, interm_input), dim=1)
        x5 = self.fc5(x4)
...
class Decoder(nn.Module):
    def __init__(self, latent_size, warper_kargs, generator_kargs, decoder_kargs):
    ...
    def forward(self, input, output_warped_points=False, output_warping_param=False,
                output_warped_points_only=False, step=1.0, generator=False):
        p_final, warping_param, warped_xyzs = self.warper(input, step=step)

        if output_warped_points_only:
            return p_final
        else:
            if generator:
                xs = []
