...
def calc_metric(metric, source_file, target_points, target_faces):
    source_points, source_faces = pcu.load_mesh_vf(source_file)
    if metric == 'chamfer':
        dist = pytorch3d.loss.chamfer_distance(torch.from_numpy(source_points).unsqueeze(0).cuda(), target_points.cuda())[0].detach().cpu().item()
    elif metric == 'emd':
        dist = calc_earth_movers_distance(source_points, target_points[0].cpu().numpy())
        ...
        dist = mesh_comp(source_mesh, target_points[0].cpu().numpy())
    return dist


def main_function(source_file_dir, target_file_dir, split_file, metric, pattern=None, missing=False, noise=False, template=False):
...
if __name__ == "__main__":
    ...
    main_function(args.data_source, args.data_target, args.split_file, args.metric, args.pattern, args.missing, args.noise, args.template)
