...
from metrics.chamfer import*


def create_mesh(
    decoder, latent_vec, filename, N=256, max_batch=(32 ** 3 * 4), offset=None, scale=None, volume_size=2.0, save_voxel=False,sdf_data=None
):
    start = time.time()
...
def create_mesh_octree(
        ...
        volume_size=2.0, save_voxel=False, sdf_data=None):
    start = time.time()
    ply_filename = filename
    ...
    while reso > 0:
        ...
        while head < samples_.shape[0]:
            ...
            if sdf_data is not None:
                _, idx = ChamferDistance()(samples_[query_idx, :3].unsqueeze(0), sdf_data[:, :3].unsqueeze(0))
                s = sdf_data[idx[0], 3].squeeze()
            else:
