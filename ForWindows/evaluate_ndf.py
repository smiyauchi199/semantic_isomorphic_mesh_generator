...
def main_function(source_file_dir, target_file_dir, split_file, metric, pattern=None, missing=False, noise=False, template=False, threshold=0.01):
    ...
    if template:
        ...
        json_data = {}
        for target_points, target_faces, instance_name in tqdm(dataloader):
            source_file = os.path.join(source_file_dir, instance_name[0] + '_1_fine_correspondence.ply')
...
if __name__ == "__main__":
    ...
    threshold = float(args.threshold) if args.threshold is not None else 0.01

    main_function(args.data_source, args.data_target, args.split_file, args.metric, args.pattern, args.missing, args.noise, args.template,     threshold = float(args.threshold) if args.threshold is not None else 0.01
)
