...
def code_to_mesh(experiment_directory, checkpoint, start_id, end_id,
    ...
    train_split_file = specs["TempSplit"]
    ...
    for i, instance_filename in enumerate(instance_filenames):
        ...
        # instance_filenameのパス部分を消去し、指定パスに置き換える
        npz_filename = os.path.basename(instance_filename).replace('.npz', '.pth')
        latent_vector_path = os.path.join(
            r"./3DModelDataset/ShapeNet/master-results/proposed/bathtub/reconstruction/Codes",
            npz_filename
        )

        latent_vector = load_latent_vectors(latent_vector_path)
