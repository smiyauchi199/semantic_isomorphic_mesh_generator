...
# How to replicate a paper result using Windows
## 1. Technical Details
...
## 2. Pretrained Network Weights (Reproducing Results Without Training)
...
- Input files generated using the pretrained network under:  
  - `data/Codes`  
  - `data/PointCloudSdfSamples`
    
These **input files are the ones required in the next step 3** to reproduce the results shown in the paper.  

**Note:**  
These pretrained weights were trained on a workstation equipped with **4× NVIDIA RTX A6000 GPUs (OS: Linux)**.  

---

## 3. Command to Reproduce Paper Results

To reproduce **Figure 4: Bathtub generated by the proposed method** in the paper using the pretrained weights and template mesh on the Windows PC described in Section 1, run:

```bash
python reconstruct_structured_meshes.py \
    -d ./data \
    -c latest \
    -m f9fe136c1738dcb1ec3cc26553cfa06b \
    --batch_split 2 \
    -e examples/bathtubs_dit \
    -s examples/splits/sv2_bathtubs_test.json
```

The final result (Bathtub generated by the proposed method) will be saved under:
./data/Meshes
